...
from app.accounts.models import AccountCreate, AccountType
from app.accounts.services import create_account, get_account_by_email
...
def load_data(session: Session) -> None:
    ...
    for u in users:
        logger.info(f"\tCreating user={u.email}, type={u.account_type}")
        ...
        if db_account:
            ...
            # ).first()
            logger.info(f"User created: {created_account}")
